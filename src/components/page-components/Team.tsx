...
const Team = () => {
  const teamMembers = [
    ...
    
    {
      ...
      image: "/assets/images/coaches/coach5.jpg",
      socials: {
        linkedin: "https://linkedin.com/in/cynthiaumwali",
          ...
          {teamMembers.map((member, index) => (
            ...
            >
              ...
              <div className="p-3 flex flex-col justify-between flex-1">
                <div>
                  <h3 className="text-base font-bold text-olympiad-navy">{member.name}</h3>
                  <p className="text-olympiad-blue font-medium text-xs">{member.role}</p>
                </div>
                
                <div className="flex items-center space-x-3 mt-2">
                  {member.socials.linkedin && (
                    <Link href={member.socials.linkedin} className="text-gray-500 hover:text-olympiad-blue transition-colors">
                      <Linkedin size={16} />
                    </Link>
                  )}
                  {member.socials.email && (
                    <Link href={`mailto:${member.socials.email}`} className="text-gray-500 hover:text-olympiad-blue transition-colors">
                      <Mail size={16} />
                    </Link>
                  )}
                </div>
